...
beta_env_values = [0.000012, 0.000014, 0.000016];
...
xlabel('Time (days)', 'Interpreter', 'latex', 'FontSize', 30);
ylabel('$I_c$', 'Interpreter', 'latex', 'FontSize', 30);
title('Effect of $\beta_{\rm env}$ on Infected Cattle ($I_c$)', ...
    'Interpreter', 'latex', 'FontSize', 18);
...
for b = 1:length(beta_env_values)
    ...
    [t, y] = ode45(@(t, y) ode_system(t, y, p_sample), tspan, y0);
    plot(t, y(:,3), 'LineWidth', 3, 'Color', colors_env(b,:));
end
...
% % === Vary theta_b for I_b ===
theta_b_values = [0.05, 0.28, 0.5];
colors_b = lines(length(theta_b_values));
figure('Name', 'I_b vs theta_b');
hold on; grid on;
xlabel('Time (days)', 'Interpreter', 'latex', 'FontSize', 16);
ylabel('$I_b$', 'Interpreter', 'latex', 'FontSize', 16);
title('Effect of $\theta_{b}$ on Infected Birds ($I_b$)', ...
    'Interpreter', 'latex', 'FontSize', 18);

for b = 1:length(theta_b_values)
    p_sample = params;
    p_sample.beta_env = theta_b_values(b);

    [t, y] = ode45(@(t, y) ode_system(t, y, p_sample), tspan, y0);
    plot(t, y(:,5), 'LineWidth', 2, 'Color', colors_env(b,:));
end
legend(arrayfun(@(x) sprintf('$\\theta_{b} = %.5f$', x), theta_b_values, 'UniformOutput', false), ...
    'Interpreter', 'latex', 'FontSize', 14, 'Location', 'best');



% === Vary theta_c for B ===
theta_c_values = [0.05, 0.2, 0.7];
...
for b = 1:length(theta_c_values)
    ...
    [t, y] = ode45(@(t, y) ode_system(t, y, p_sample), tspan, y0);
    plot(t, y(:,6), 'LineWidth', 3, 'Color', colors_b(b,:));
end
...
gamma_c_values = [0.961, 0.982, 1.043];
...
for b = 1:length(gamma_c_values)
    ...
    [t, y] = ode45(@(t, y) ode_system(t, y, p_sample), tspan, y0);
    plot(t, y(:,3), 'LineWidth', 3, 'Color', colors_b(b,:));
end
legend(arrayfun(@(x) sprintf('$\\gamma_c = %.6f$', x), gamma_c_values, 'UniformOutput', false), ...
    'Interpreter', 'latex', 'FontSize', 16, 'Location', 'best');

% === Vary mu_env for B ===
mu_env_values = [0.054, 0.06, 0.08];
colors_b = lines(length(mu_env_values));

figure('Name', 'B vs mu_env');
hold on; grid on;
xlabel('Time (days)', 'Interpreter', 'latex', 'FontSize', 25);
...
legend(arrayfun(@(x) sprintf('$\\mu_{env} = %.3f$', x), mu_env_values, 'UniformOutput', false), ...
    'Interpreter', 'latex', 'FontSize', 16, 'Location', 'best');
